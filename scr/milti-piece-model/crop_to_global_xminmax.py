...
class MultiPieceCropperConfig:
    def __init__(
        ...
        out_dir: Path = Path("multi_piece_dataset"),
        image_exts: Tuple[str, ...] = (".jpg", ".jpeg", ".png"),
        margin_percent: float = 0.05,  # +5%
        set_folder_tag: str = "multi-piece-train-cropped",
...
class MultiPieceXmlCropper:
    ...
    def update_size_block(self, root: ET.Element, new_w: int, new_h: int) -> None:
        ...
        depth_el.text = depth_el.text.strip() if (depth_el.text and depth_el.text.strip()) else self.cfg.depth_value

    def crop_and_update_one(
        self,
        xml_in: Path,
        img_in: Path,
        xml_out: Path,
        ...
        with Image.open(img_in) as im:
            ...
            cropped = im.crop((cl, 0, cr, h))
            cropped.save(img_out)

        # Update XML
        tree, root, boxes = self.parse_boxes(xml_in)

        self.ensure_child(root, "folder").text = self.cfg.set_folder_tag
        self.ensure_child(root, "filename").text = img_out.name
        self.ensure_child(root, "path").text = self.cfg.set_path_tag

        self.update_size_block(root, new_w, new_h)

        # Shift & clamp boxes; drop invalid
        to_remove = []
    ...
    def run(self) -> None:
        ...
        # Pass 1: global min/max from all boxes
        for xml_path in sorted(self.cfg.src_dir.glob("*.xml")):
            ...
            if not boxes:
                print(f"WARNING: No boxes in {xml_path.name}")
                continue
        ...
        # Pass 2: crop + update all files
        converted = 0
        for xml_in, img_in in pairs:
            if img_in.suffix.lower() in (".jpg", ".jpeg"):
                img_out = self.cfg.out_dir / f"{xml_in.stem}.jpg"
            else:
                img_out = self.cfg.out_dir / f"{xml_in.stem}{img_in.suffix.lower()}"

            xml_out = self.cfg.out_dir / f"{xml_in.stem}.xml"

            self.crop_and_update_one(
                xml_in=xml_in,
                img_in=img_in,
                xml_out=xml_out,
                img_out=img_out,
        ...
        print(f"Done. Saved {converted} cropped pairs into: {self.cfg.out_dir.resolve()}")